...
/**
 * Standalone Conflict Style Assessment
 *
 ...
 *   CREATE POLICY "Users can manage their own conflict assessment" ON conflict_assessments
...
import { ASSESSMENT_MODULES, ASSESSMENT_QUESTIONS, scoreConflictStyle } from '@/lib/relationship-questions'

const MODULE = ASSESSMENT_MODULES.find(m => m.id === 'conflict_style')
const QUESTIONS = ASSESSMENT_QUESTIONS.conflict_style

export default function ConflictAssessmentPage() {
  ...
  const handleNext = useCallback(() => {
    ...
    if (currentIndex < QUESTIONS.length - 1) {
      ...
      const scored = scoreConflictStyle(answers)
      setResult(scored)
      setPhase('results')
    }
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (error) console.error('Conflict save error:', error)
      setSaved(true)
    } catch (err) {
      console.error('Conflict save catch:', err)
            ...
            <div className="p-8 flex flex-col flex-1">
              ...
              <div className="mb-8">
                ...
                <ul className="space-y-2">
                  {['Primary conflict style (Validator, Volatile, Avoider, or Reactive)', 'How your style shows up when things get heated', 'Strengths of your style and your personal growth edge', 'A coach insight tailored to your pattern'].map(item => (
    ...
    return (
      <div className="min-h-screen bg-[#F8F6F3]">
        ...
        <div className="fixed top-0 left-0 right-0 h-1 bg-[#E5E2DD] z-50">
          <div className="h-full bg-gradient-to-r from-[#E8614D] to-[#c94a39] transition-all duration-500" style={{ width: `${progress}%` }} />
        </div>
        ...
        <div className="max-w-2xl mx-auto px-4 py-12">
          ...
          <div className="bg-white rounded-2xl shadow-sm p-8 border border-[#E5E2DD] mb-8">
            ...
            <div className="space-y-3">
              {currentQuestion.options.map((option) => {
                ...
                return (
                  <button key={option.value} onClick={() => handleSelect(option)} className={`w-full text-left p-4 rounded-xl border-2 transition-all ${isSelected ? 'border-[#E8614D] bg-[#FDF6EF]' : 'border-[#E5E2DD] hover:border-[#E8614D]/50 hover:bg-[#F8F6F3]'}`}>
                    <div className="flex items-center gap-4">
                      <div className={`w-6 h-6 rounded-full border-2 flex items-center justify-center flex-shrink-0 transition-all ${isSelected ? 'border-[#E8614D] bg-[#E8614D]' : 'border-[#E5E2DD]'}`}>
          ...
          <div className="flex items-center justify-between">
            ...
            <button onClick={handleNext} disabled={!selectedOption} className="flex items-center gap-2 bg-gradient-to-r from-[#E8614D] to-[#c94a39] text-white px-6 py-3 rounded-full font-semibold disabled:opacity-40 disabled:cursor-not-allowed hover:opacity-90 transition-opacity">
        ...
        <div className="max-w-2xl mx-auto px-4 py-12">
          ...
          <div className="bg-white rounded-3xl shadow-lg overflow-hidden mb-6">
            <div className="p-8 text-center" style={{ background: `linear-gradient(135deg, ${profile.color}22, ${profile.color}11)`, borderBottom: `4px solid ${profile.color}` }}>
              ...
              <div className="inline-block px-3 py-1 rounded-full text-sm font-semibold mb-3" style={{ background: `${profile.color}22`, color: profile.color }}>Your Conflict Style</div>
                ...
                <div className="space-y-2">
                  {Object.entries(counts).sort((a, b) => b[1] - a[1]).map(([style, count]) => (
                    <div key={style} className="flex items-center gap-3">
                      <span className="w-32 text-sm text-[#6B7280] text-right">{styleLabels[style]}</span>
