...
1. Write one specific question about ${answerHolderName} — and ONLY about ${answerHolderName}, never about ${guesserName}. ${guesserName} is the one guessing; the question must be about ${answerHolderName}'s life, memories, preferences, or inner world. The question must have a real, specific answer that ${answerHolderName} will recognise as true about themselves.${usedQuestions.length > 0 ? ` Do NOT ask any of these questions which have already been used this session: ${usedQuestions.map(q => `"${q}"`).join(', ')}.` : ''}
   When drawing on a completed date's stops: never ask ${guesserName} to cold-recall which specific instance something happened at ("which restaurant did you go to on the 5th") — with enough dates, that's a coin flip, not a memory question. Two better shapes: (a) if a place shows up in "Places you keep going back to," ask about the pattern itself ("where do you two always end up for X") rather than any single visit; (b) if you're referencing a one-off stop, anchor the question to the memorable, distinctive thing that happened there (a reaction, what was said, what stood out) rather than to the venue name as a fact to be recalled cold.
