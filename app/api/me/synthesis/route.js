...
export async function POST(request) {
  try {
    ...
    // Find users with notebook entries in the last 7 days
    ...
    if (!activeEntries?.length) return NextResponse.json({ processed: 0 })
    ...
    for (const userId of userIds) {
      try {
        // Fetch last 7 days of entries, user's nora notes, active practices, and couple id
        const [
          { data: entries },
          { data: practices },
          { data: coupleRow },
        ...
        // Fetch user's person notes from nora_memory if couple exists
        let userNotes = null
        if (userCoupleId) {
          ...
          if (memory) {
            const { data: coupleData } = await supabase
              ...
              .eq('id', userCoupleId)
              .single()
            const notes = coupleData?.user1_id === userId ? memory.user1_notes : memory.user2_notes
        ...
        console.error(`[me/synthesis] Error processing user ${userId}:`, userErr)
      }
    }
