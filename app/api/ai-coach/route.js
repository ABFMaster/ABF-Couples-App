  ...
  try {
    ...
    try {
      ...
      [{ data: userProfile }, { data: partnerProfile }] = await Promise.all([
        supabase
          .from('user_profiles')
          .select('display_name, attachment_style, conflict_style, conflict_secondary, love_language_primary, love_language_profile, attachment_anxiety_score, attachment_avoidance_score, flooding_prone, repair_style, assessment_completed_at, hobbies, date_preferences, birthday')
          .eq('user_id', user.id)
          .maybeSingle(),
        partnerId
          ? supabase
    ...
    const claimsBlock = coupleRow
      ...
      : ''
    console.log('[ai-coach][TEST] claimsBlock content:', claimsBlock)
