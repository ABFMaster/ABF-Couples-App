...
- 🔴 Real privacy finding, NOT fixed yet, deliberately excluded from the Dates/Flirts wiring: AI coach conversations (SIGNAL_TYPES.NORA_CONVERSATION) currently do feed into the couple-shared `memory_summary` that Memory Test (and likely other couple-facing Nora surfaces) reads. Traced the full path in lib/nora-memory.js: NORA_CONVERSATION is correctly excluded from `SHARED_SIGNALS` (so it never writes directly to `couple_notes`), but it IS in `INDIVIDUAL_NOTE_SIGNALS`, so a private AI coach conversation gets synthesized into that person's `user1_notes`/`user2_notes` — and `buildMemorySummaryPrompt` then blends BOTH partners' individual notes (whatever signal type they came from) into the one shared `memory_summary`, with no instruction in that prompt to keep one-sided disclosures confidential. So a private thing said to the AI coach could end up paraphrased into the summary both partners' Memory Test questions (and other surfaces) draw on. Matt flagged this directly: "The individual is sharing individually with Nora and that content should not be used." This needs a dedicated session, not a rushed fix — `user1_notes`/`user2_notes` are currently a blend of many different signal types with no source-level separation, so a real fix means either genuinely separating AI-coach-sourced notes into their own field never passed to buildMemorySummaryPrompt, or reworking the notes schema to tag content by source. Either touches lib/nora-memory.js, which every couple-facing Nora surface in the app depends on — worth its own careful pass, same as the Memory Test gate was.
