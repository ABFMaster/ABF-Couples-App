...
- 🟡 Follow-Through weekly rollout mapped July 29 — see Sessions/FOLLOW_THROUGH_WEEKLY_ROLLOUT.md. Spark (Monday) is lowest-risk next step, needs one contained addition (a reveal-seen flag, since Spark has no tap-based reveal to key off like Bet does). Wednesday (Notice) and Thursday both reveal on a fixed clock (7pm) rather than on both-answered like Bet/Spark — genuinely different trigger shape, needs a dedicated read of the reveal-computation code before either can be scoped, not a guess. Friday (Ritual) is the one real finding: it does not fit the mechanic structurally (weekly not daily, one shared signal not two independent ones) and forcing it in would work against the design rather than for it — a ritual's own weekly check-in already is real-world-action-plus-report, so it's exempt by design, not a gap.
