...
//
// Params:
//   sourceType    — 'bet' | 'spark' | ... (stored on the row, read back by
//                   /api/follow-through/today's hasSeenSourceReveal gate)
//   sourceId      — the bet/spark id
//   sourceLabel   — display name used in generated prompts, e.g. "Bet", "Spark"
//   sourceQuestion — the question text asked that day
...
export async function generateFollowThrough({
  ...
  const buildBasePrompt = (forName, otherName, forAnswer, otherAnswer) => `The ${sourceLabel} question tonight was: "${sourceQuestion}"
...
Bias toward OTHER-DIRECTED unless tonight's content specifically calls for private reflection.`
  ...
  const [myResult, partnerResult] = await Promise.all([
    generateFor(myName, partnerName, myAnswer, theirAnswer, myVariant),
    generateFor(partnerName, myName, theirAnswer, myAnswer, partnerVariant),
  ])
