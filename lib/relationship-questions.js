...
export const generateModuleInsights = (moduleId, answers) => {
  if (moduleId === 'attachment_profile') {
    ...
    if (!anxietyHigh && !avoidanceHigh) {
      ...
      style = 'fearful'; headline = 'Fearfully Attached'; description = "You want closeness and feel wary of it at the same time. This is one of the most human experiences there is — and one I can help with."
    }
...
export function scoreAttachmentStyle(answers) {
  ...
  if (!anxietyHigh && !avoidanceHigh) {
    primary = 'secure'
    profile = { label: 'Secure', emoji: '🌿', tagline: 'Comfortable with closeness and independence', summary: 'You move toward intimacy with ease and handle distance without alarm.', strengths: ['Emotionally available', 'Handles conflict without catastrophizing', 'Trusts without excessive reassurance-seeking'], growth: 'Staying attuned to partners with different attachment patterns', coachInsight: 'Your security is a resource for the relationship. I will help you use it well.', color: '#7A8C6E' }
  } else if (anxietyHigh && !avoidanceHigh) {
    primary = 'anxious'
    profile = { label: 'Anxious', emoji: '🌊', tagline: 'Deeply attuned, quickly activated', summary: "You feel connection deeply and notice disconnection quickly.", strengths: ['Highly attuned to partner', 'Invests fully in relationships', 'Emotionally expressive'], growth: 'Learning to sit with uncertainty without interpreting silence as distance', coachInsight: "Your attunement is a gift. I will help you trust it without letting it run you.", color: '#C4714A' }
  } else if (!anxietyHigh && avoidanceHigh) {
    primary = 'avoidant'
    profile = { label: 'Avoidant', emoji: '🏔️', tagline: 'Self-sufficient, wary of dependence', summary: "You value independence and handle your own emotional world well.", strengths: ['Self-sufficient', 'Low reactivity under pressure', 'Brings calm to charged situations'], growth: 'Letting someone in without experiencing it as a loss of self', coachInsight: "Your steadiness is real. I will help you use it to connect, not just to cope.", color: '#8B7355' }
  } else {
    primary = 'fearful'
    profile = { label: 'Fearful', emoji: '🌓', tagline: 'Wants closeness, wary of it', summary: "You want closeness and feel wary of it at the same time.", strengths: ['Deep capacity for empathy', 'Understands relational complexity', 'Highly self-aware when supported'], growth: 'Building enough safety to stay present when intimacy feels threatening', coachInsight: "This is one of the most human experiences there is. I will move carefully with you.", color: '#6B5D4F' }
  }
