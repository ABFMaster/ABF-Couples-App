...
export async function buildCoachContext(userId, coupleId, supabase) {
  ...
  try {
    ...
    // relationship_health has no `score` column, only `overall_score`. The
    // old select mixed a real and a fake column, which fails the whole
    // query — healthScore silently never made it into Nora's AI Coach
    // context.
    ...
    if (health) {
      context.relationship.healthScore = health.overall_score ?? null;
    }
  } catch (e) {
  ...
  try {
    ...
    // ROOT CAUSE FIX Aug 21 2026 — found via full schema-drift audit:
    // date_plans has no `notes` column, only `reflection_notes`. The old
    // select silently failed the whole plannedRes query (Promise.allSettled
    // still resolves "fulfilled" on a Supabase query error — it only
    // captures thrown exceptions, not {data: null, error} responses — so
    // this was never visible as a rejection), meaning past date_plans
    // entries never made it into Nora's completed-dates context at all,
    // not just the notes field.
    const [plannedRes, customRes, nextPlannedRes, nextCustomRes] = await Promise.allSettled([
      supabase.from('date_plans').select('title, date_time, reflection_notes').eq('couple_id', coupleId).in('status', ['planned', 'accepted']).lt('date_time', now).order('date_time', { ascending: false }).limit(3),
    ...
    const completed = [
      ...plannedDates.map(d => ({ title: d.title, date: d.date_time, notes: d.reflection_notes })),
  ...
  // Attachment/conflict style fetches used to live here (sections 15/16),
  // querying attachment_assessments/conflict_assessments. Removed Aug 5
  // 2026 -- see the matching comment in formatContextForPrompt() below for
  // the full explanation (confirmed-dead tables, real data already
  // reaches Nora correctly via getNoraBriefing() elsewhere).
...
export function formatContextForPrompt(context) {
  ...
  // Attachment/conflict pairing-dynamic content used to live here, sourced
  // from context.attachmentStyle/conflictStyle. Removed Aug 5 2026 (AI Coach
  // audit) as confirmed dead code -- it queried attachment_assessments/
  // conflict_assessments tables that don't exist in the live schema (no
  // migration ever created them; confirmed via full-codebase search), so
  // context.attachmentStyle/conflictStyle were always null and this block
  // never once rendered for any couple. Also would have had a vocabulary
  // bug even if wired to the right table: conflictDescriptions expected
  // validator/avoider/hostile, but scoreConflictStyle() (lib/relationship-
  // questions.js) only ever produces volatile/validating/avoiding.
  // The equivalent — and actually-working — capability already exists via
  // getNoraBriefing() (lib/nora-knowledge.js), called separately below as
  // assessmentBriefing in app/api/ai-coach/route.js, reading the real data
  // from user_profiles.attachment_style/conflict_style with matching
  // vocabulary throughout. No functional loss from this removal.
