...
function parseLoveLanguages(rankObj) {
  ...
  return Object.entries(rankObj)
    ...
    .sort((a, b) => a[1] - b[1])  // ASCENDING: 1 = primary (ranked 1st place in quiz)
...
export async function buildCoachContext(userId, coupleId, supabase) {
  ...
  try {
    const { data: userProfile } = await supabase
      .from('user_profiles')
      .select('display_name, preferred_checkin_time, hobbies, date_preferences, love_language_primary')
      .eq('user_id', userId)
      .maybeSingle();
  ...
  try {
    ...
    const [plannedRes, customRes, nextPlannedRes, nextCustomRes] = await Promise.allSettled([
      ...
      supabase.from('custom_dates').select('title, created_at').eq('couple_id', coupleId).lt('created_at', now).neq('status', 'pending_delete').order('created_at', { ascending: false }).limit(3),
      ...
      supabase.from('custom_dates').select('title, created_at').eq('couple_id', coupleId).gte('created_at', now).neq('status', 'pending_delete').order('created_at', { ascending: true }).limit(1),
    ]);
    ...
    const completed = [
      ...
      ...customDates.map(d => ({ title: d.title, date: d.created_at })),
    ].sort((a, b) => new Date(b.date) - new Date(a.date)).slice(0, 3);

    const nextRaw = [
      ...nextPlannedDates.map(d => ({ title: d.title, date: d.date_time })),
      ...nextCustomDates.map(d => ({ title: d.title, date: d.created_at })),
    ].sort((a, b) => new Date(a.date) - new Date(b.date))[0] || null;

    context.dates = {
      completed,
      upcoming: nextRaw ? { title: nextRaw.title, date: nextRaw.date } : null,
    };
  } catch (e) {
  ...
  // ── 10. RELATIONSHIP ACTIVITY LOG ────────────────────────────────
  // relationship_points is a silent behavioral activity log.
  // NEVER show point totals or action counts to users.
  // Used by coach for pattern recognition only.
  try {
    const { data: activityLog } = await supabase
      .from('relationship_points')
      .select('action, points, created_at, user_id')
      .eq('couple_id', coupleId)
      .order('created_at', { ascending: false })
      .limit(20);

    if (activityLog?.length > 0) {
      context.activityLog = activityLog;

      const actionCounts = activityLog.reduce((acc, row) => {
        acc[row.action] = (acc[row.action] || 0) + 1;
        return acc;
      }, {});

      context.activitySummary = Object.entries(actionCounts)
        .map(([action, count]) => `${action}: ${count}x`)
        .join(', ');

      console.log('[Coach] Activity summary:', context.activitySummary);
    }
  } catch (e) {
    console.error('[Coach] Failed to fetch activity log:', e.message);
  }
  ...
  try {
    const { data: recentTrips } = await supabase
      .from('trips')
      .select('destination, trip_type, start_date, end_date, status, is_dream')
      .eq('couple_id', coupleId)
      .eq('is_dream', false)
      .order('start_date', { ascending: false })
      .limit(5);
    if (recentTrips?.length > 0) context.recentTrips = recentTrips;
    console.log('[Coach] Recent trips:', recentTrips?.length || 0);
  } catch (e) {
    console.error('[Coach] Failed to fetch trips:', e.message);
  }

  // ── 13. RECENT TIMELINE EVENTS ────────────────────────────────────
  try {
    const { data: timelineEvents } = await supabase
      ...
      .limit(5);
    if (timelineEvents?.length > 0) context.timelineEvents = timelineEvents;
    console.log('[Coach] Timeline events:', timelineEvents?.length || 0);
  } catch (e) {
    console.error('[Coach] Failed to fetch timeline events:', e.message);
  }
  ...
  try {
    const { data: recentSparks } = await supabase
      ...
      .limit(5)
    if (recentSparks?.length > 0) context.recentSparks = recentSparks
  } catch (e) {
    console.error('[Coach] Failed to fetch recent sparks:', e.message);
  }
  ...
  try {
    const { data: recentBets } = await supabase
      ...
      .limit(5)
    if (recentBets?.length > 0) context.recentBets = recentBets
  } catch (e) {
    console.error('[Coach] Failed to fetch recent bets:', e.message);
  }

  // ── 16. ATTACHMENT STYLE ──────────────────────────────────────────
  try {
    const fetchAttachment = (uid) => supabase
      .from('attachment_assessments')
      .select('primary_style, secondary_style, completed_at')
      .eq('user_id', uid)
      .order('completed_at', { ascending: false })
      .limit(1)
      .maybeSingle()
    ...
    const [userAttachment, partnerAttachment] = await Promise.all([
      fetchAttachment(userId).catch(() => null),
      partnerId ? fetchAttachment(partnerId).catch(() => null) : Promise.resolve(null),
    ]);

    context.attachmentStyle = {
      user:    userAttachment    ? { primary: userAttachment.primary_style,    secondary: userAttachment.secondary_style    } : null,
      partner: partnerAttachment ? { primary: partnerAttachment.primary_style, secondary: partnerAttachment.secondary_style } : null,
    };
    console.log('[Coach] Attachment styles:', context.attachmentStyle?.user?.primary, '/', context.attachmentStyle?.partner?.primary);
  } catch (e) {
    console.error('[Coach] Failed to fetch attachment styles:', e.message);
  }

  // ── 15. CONFLICT STYLE ────────────────────────────────────────────
  try {
    const fetchConflict = (uid) => supabase
    ...
    const [userConflict, partnerConflict] = await Promise.all([
      fetchConflict(userId).catch(() => null),
      partnerId ? fetchConflict(partnerId).catch(() => null) : Promise.resolve(null),
    ]);

    context.conflictStyle = {
      user:    userConflict    ? { primary: userConflict.primary_style,    secondary: userConflict.secondary_style    } : null,
      partner: partnerConflict ? { primary: partnerConflict.primary_style, secondary: partnerConflict.secondary_style } : null,
    };
    console.log('[Coach] Conflict styles:', context.conflictStyle?.user?.primary, '/', context.conflictStyle?.partner?.primary);
  } catch (e) {
    console.error('[Coach] Failed to fetch conflict styles:', e.message);
  }
...
export function formatContextForPrompt(context) {
  ...
  parts.push(`CRITICAL: You are speaking directly to ${userName}. ${partnerName} is their partner. Never confuse who said what. Every answer below is explicitly labeled by name.`);
  ...
  const attachmentDescriptions = {
    secure:   'Comfortable with intimacy and independence. Tends to be emotionally available and resilient under stress.',
    anxious:  'Craves closeness but fears abandonment. May seek reassurance frequently and amplify conflict when feeling disconnected.',
    avoidant: 'Values independence and self-sufficiency. May pull away emotionally when feeling overwhelmed or pressured.',
    fearful:  'Desires closeness but fears being hurt. Can oscillate between seeking and avoiding intimacy.',
  };
  const conflictDescriptions = {
    validator: 'Calm and empathetic in disagreements. Validates feelings, de-escalates naturally, and listens well.',
    volatile:  'Emotionally expressive in conflict. Passionate and direct but can escalate quickly.',
    avoider:   'Sidesteps conflict and prioritizes harmony. May leave issues unresolved.',
    hostile:   'Can become reactive or critical under stress. Tends to defend strongly and may escalate.',
  };

  const userAtt = context.attachmentStyle?.user;
  const partnerAtt = context.attachmentStyle?.partner;
  const userConf = context.conflictStyle?.user;
  const partnerConf = context.conflictStyle?.partner;

  if (userAtt?.primary) {
    parts.push(`\n${userName}'s attachment style: ${userAtt.primary.charAt(0).toUpperCase() + userAtt.primary.slice(1)}${userAtt.secondary ? ` (secondary: ${userAtt.secondary})` : ''} — ${attachmentDescriptions[userAtt.primary] || ''}`);
  }
  if (userConf?.primary) {
    parts.push(`${userName}'s conflict style: ${userConf.primary.charAt(0).toUpperCase() + userConf.primary.slice(1)}${userConf.secondary ? ` (secondary: ${userConf.secondary})` : ''} — ${conflictDescriptions[userConf.primary] || ''}`);
  }
  if (partnerAtt?.primary) {
    parts.push(`${partnerName}'s attachment style: ${partnerAtt.primary.charAt(0).toUpperCase() + partnerAtt.primary.slice(1)}${partnerAtt.secondary ? ` (secondary: ${partnerAtt.secondary})` : ''} — ${attachmentDescriptions[partnerAtt.primary] || ''}`);
  }
  if (partnerConf?.primary) {
    parts.push(`${partnerName}'s conflict style: ${partnerConf.primary.charAt(0).toUpperCase() + partnerConf.primary.slice(1)}${partnerConf.secondary ? ` (secondary: ${partnerConf.secondary})` : ''} — ${conflictDescriptions[partnerConf.primary] || ''}`);
  }

  // Pairing insights
  if (userAtt?.primary && partnerAtt?.primary) {
    const attachPair = [userAtt.primary, partnerAtt.primary].sort().join('_');
    const attachPairingInsights = {
      anxious_avoidant: `PAIRING DYNAMIC: ${userName} (anxious) + ${partnerName} (avoidant) — classic pursuer-withdrawer cycle. When ${userName} reaches for connection, ${partnerName} may pull back. Acknowledge this pattern gently if it surfaces in check-ins.`,
      anxious_fearful:  `PAIRING DYNAMIC: Both styles can create push-pull cycles with different textures. Focus on building safety rituals that reduce both partners' nervous system activation.`,
      avoidant_fearful: `PAIRING DYNAMIC: Both lean toward emotional distance under stress, which can create disconnection without conflict. Help them recognize the quiet drift.`,
      secure_anxious:   `PAIRING DYNAMIC: ${partnerName ? (partnerAtt.primary === 'anxious' ? partnerName : userName) : 'secure partner'} can be a stabilizing anchor. Encourage the secure partner to offer proactive reassurance.`,
      secure_avoidant:  `PAIRING DYNAMIC: The secure partner can model emotional availability without pressure. Gradual trust-building is key.`,
      secure_fearful:   `PAIRING DYNAMIC: Secure partner provides a safe base. Steady, consistent presence matters more than intensity here.`,
      secure_secure:    `PAIRING DYNAMIC: Both securely attached — strong foundation for direct communication and mutual support.`,
      anxious_anxious:  `PAIRING DYNAMIC: Two anxious styles can amplify each other's fears. Encourage independent soothing practices and clear communication rituals.`,
      avoidant_avoidant: `PAIRING DYNAMIC: Both value space — risk of emotional distance becoming the norm. Help them identify low-stakes ways to initiate connection.`,
      fearful_fearful:  `PAIRING DYNAMIC: Both partners carry deep ambivalence about closeness. Safety, predictability, and no-pressure rituals are essential.`,
    };
    const pairInsight = attachPairingInsights[attachPair];
    if (pairInsight) parts.push(`\n${pairInsight}`);
  }

  if (userConf?.primary && partnerConf?.primary) {
    const conflictPair = [userConf.primary, partnerConf.primary].sort().join('_');
    const conflictPairingInsights = {
      avoider_volatile: `CONFLICT DYNAMIC: ${userConf.primary === 'volatile' ? userName : partnerName} escalates while ${userConf.primary === 'avoider' ? userName : partnerName} withdraws — a classic mismatch. One partner's intensity can drive the other further away. Suggest structured cool-down agreements.`,
      hostile_volatile: `CONFLICT DYNAMIC: Two high-reactivity styles risk rapid escalation. The 20-minute physiological self-soothing rule (Gottman) is critical here.`,
      avoider_hostile:  `CONFLICT DYNAMIC: Hostile partner's criticism can intensify avoider's withdrawal. Work on softened start-ups and explicit invitations to re-engage.`,
      validator_volatile: `CONFLICT DYNAMIC: Complementary — ${userConf.primary === 'validator' ? userName : partnerName}'s calm can regulate ${userConf.primary === 'volatile' ? userName : partnerName}'s intensity. Encourage the validator to hold steady without dismissing feelings.`,
      avoider_validator: `CONFLICT DYNAMIC: Both tend toward harmony — may avoid necessary repair conversations. Encourage scheduled "relationship check-in" time.`,
      hostile_validator: `CONFLICT DYNAMIC: The validator's patience is a resource, but chronic criticism from the hostile partner can erode it. Name the pattern if it appears.`,
      validator_validator: `CONFLICT DYNAMIC: Low-conflict but may suppress important issues. Normalize productive disagreement.`,
      volatile_volatile: `CONFLICT DYNAMIC: Both are passionate and expressive — arguments can be intense but also repair quickly. Emphasize repair rituals.`,
      avoider_avoider:  `CONFLICT DYNAMIC: Harmony-seeking pair who may never address core issues. Gentle structured conversations are essential.`,
      hostile_hostile:  `CONFLICT DYNAMIC: High mutual defensiveness. Work on softened start-ups and de-escalation agreements before any difficult topic.`,
    };
    const cpInsight = conflictPairingInsights[conflictPair];
    if (cpInsight) parts.push(cpInsight);
  }
  ...
  if (context.recentTrips?.length > 0) {
    const tripSummary = context.recentTrips
      .map(t => {
        const dest = t.destination || 'Unknown';
        const type = t.trip_type ? ` (${t.trip_type})` : '';
        const status = t.status ? ` — ${t.status}` : '';
        const dateStr = t.start_date ? ` · ${new Date(t.start_date).toLocaleDateString('en-US', { month: 'short', year: 'numeric' })}` : '';
        return `${dest}${type}${dateStr}${status}`;
      })
      .join('; ');
    parts.push(`\nRecent trips: ${tripSummary}`);
  }

  // ── TIMELINE EVENTS ───────────────────────────────────────────────
  if (context.timelineEvents?.length > 0) {
    const eventSummary = context.timelineEvents
      .map(e => {
        const dateStr = e.event_date ? new Date(e.event_date).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' }) : '';
        return `"${e.title}"${dateStr ? ` on ${dateStr}` : ''}`;
      })
      .join(', ');
    parts.push(`Recent timeline moments: ${eventSummary}`);
  }

  // ── SPARK & BET HISTORY ───────────────────────────────────────────
  if (context.recentSparks?.length > 0 || context.recentBets?.length > 0) {
    parts.push(`\nRECENT SPARK & BET ANSWERS:`);
    parts.push(`(These are real answers from ${userName} and ${partnerName} — use their names explicitly when referencing these)`);

    if (context.recentSparks?.length > 0) {
      context.recentSparks.forEach(spark => {
        if (!spark.spark_responses?.length) return;
        parts.push(`\nSpark (${new Date(spark.spark_date).toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}): "${spark.question}"`);
        spark.spark_responses.forEach(r => {
          const responderName = r.user_id === context.user?.id ? userName : partnerName;
          if (r.response_text) {
            parts.push(`  ${responderName} said: "${r.response_text}"`);
          }
        });
      });
    }

    if (context.recentBets?.length > 0) {
      context.recentBets.forEach(bet => {
        if (!bet.bet_responses?.length) return;
        parts.push(`\nBet (${new Date(bet.bet_date).toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}): "${bet.question}"`);
        bet.bet_responses.forEach(r => {
          const responderName = r.user_id === context.user?.id ? userName : partnerName;
          if (r.prediction) parts.push(`  ${responderName} predicted: "${r.prediction}"`);
          if (r.actual_answer) parts.push(`  ${responderName}'s actual answer: "${r.actual_answer}"`);
        });
      });
    }
  }
  ...
  if (context.activitySummary) {
    parts.push(`\nRelationship Activity (recent): ${context.activitySummary}`);
  }
