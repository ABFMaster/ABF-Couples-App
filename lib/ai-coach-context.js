...
export async function buildCoachContext(userId, coupleId, supabase) {
  ...
  try {
    ...
    const [plannedRes, customRes, nextPlannedRes, nextCustomRes] = await Promise.allSettled([
      ...
      supabase.from('custom_dates').select('title, created_at').eq('couple_id', coupleId).lt('created_at', now).order('created_at', { ascending: false }).limit(3),
      ...
      supabase.from('custom_dates').select('title, created_at').eq('couple_id', coupleId).gte('created_at', now).order('created_at', { ascending: true }).limit(1),
    ]);
