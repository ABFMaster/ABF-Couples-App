...
export async function updateNoraMemory({ coupleId, userId, signalType, inputData }) {
  try {
    ...
    try {
      const { error: signalLogError } = await supabase.from('nora_signals').insert({
        couple_id: coupleId,
        user_id: userId || null,
      ...
      if (signalLogError) console.error('[nora-memory] nora_signals insert error:', signalLogError)
    } catch (err) {
      console.error('[nora-memory] nora_signals insert exception:', err)
    }
