...
// ─────────────────────────────────────────────────────────────────────────────
export async function getSurfaceableClaims(coupleId, user1Id, user2Id, user1Name, user2Name) {
  try {
    ...
    const formatted = claims.map(c => {
      ...
      const mode = c.confidence >= 0.85 ? 'DIRECT' : 'TENTATIVE'
  ...
  try {
    ...
    if (individualIncrement > 0 || coupleIncrement > 0) {
      ...
      if (individualIncrement > 0) {
        updateFields.individual_signal_count = (existing?.individual_signal_count || 0) + individualIncrement
      }
