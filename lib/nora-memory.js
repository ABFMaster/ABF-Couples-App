...
export async function shouldUpdateMemory(lastTwoMessages) {
  try {
    const response = await anthropic.messages.create({
      ...
      max_tokens: 10,
      system: `You are a signal detector for a couples therapy AI.
You read the last exchange in a conversation and return YES or NO only.

Return YES if any of these occurred:
- A person described what love actually feels like to them in practice (not theory)
- A clear attachment fear surfaced beneath surface behavior
- Something contradicts what we previously knew about this person
- The negative cycle between partners appeared live in the conversation
- A person revealed something about themselves they have not said before

Return NO for everything else: advice, logistics, small talk, general discussion.

Return only YES or NO. Nothing else.`,
      messages: [{ role: 'user', content: JSON.stringify(lastTwoMessages) }],
    })
...
async function extractStructuredFacts(user1Name, user2Name, user1Notes, user2Notes, coupleNotes) {
  try {
    const response = await anthropic.messages.create({
      ...
      messages: [{
        role: 'user',
        content: `Extract structured facts from these therapy notes.
...
export async function updateNoraMemory({ coupleId, userId, signalType, inputData }) {
  try {
    ...
    const [newUser1Notes, newUser2Notes, newCoupleNotes] = await Promise.all([
      updateUser1 ? anthropic.messages.create({
        model: 'claude-sonnet-4-6',
        max_tokens: 400,
        messages: [{
          role: 'user',
          content: buildPersonNotesPrompt(user1Name, signalType, inputData, existingUser1Notes)
        }],
      }).then(r => r.content[0].text.trim()) : Promise.resolve(existingUser1Notes),

      updateUser2 ? anthropic.messages.create({
        model: 'claude-sonnet-4-6',
        max_tokens: 400,
        messages: [{
          role: 'user',
          content: buildPersonNotesPrompt(user2Name, signalType, inputData, existingUser2Notes)
        }],
      }).then(r => r.content[0].text.trim()) : Promise.resolve(existingUser2Notes),

      updateCouple ? anthropic.messages.create({
        model: 'claude-sonnet-4-6',
        max_tokens: 400,
        messages: [{
          role: 'user',
          content: buildCoupleNotesPrompt(user1Name, user2Name, signalType, inputData, existingCoupleNotes, recentSignalsSummary)
        }],
      }).then(r => r.content[0].text.trim()) : Promise.resolve(existingCoupleNotes),
    ])
    ...
    const [newSummary, structuredFacts] = await Promise.all([
      anthropic.messages.create({
        ...
        messages: [{
          role: 'user',
          content: buildMemorySummaryPrompt(user1Name, user2Name, newUser1Notes, newUser2Notes, newCoupleNotes)
        }],
