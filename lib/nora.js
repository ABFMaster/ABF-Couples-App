...
// ROOT CAUSE FIX Aug 19 2026 — this was fire-and-forget (insert started,
// never awaited). Fine on the success path where there's usually more
// async work happening after it anyway, but on an error path the caller
// typically logs then returns/throws immediately — on Vercel's serverless
// runtime the function can freeze before the un-awaited insert's network
// round-trip completes. Confirmed live: flirts/generate hit a real error
// today and nora_calls had nothing from it at all. Now async + awaited by
// both call sites below; still wrapped in try/catch so a logging failure
// can never itself become the reason a Nora call fails.
async function logCall({ route, context, model, usage, latencyMs, status = 'success', stopReason, errorType, errorMessage }) {
  try {
    await supabase
      .from('nora_calls')
      .insert({
...
async function noraCall({
  ...
  // ROOT CAUSE FIX Aug 19 2026 — this line used to run unguarded and
  // OUTSIDE any try/catch. anthropic.messages.create() can succeed at the
  // HTTP level but still return a response with no usable text block (rare
  // edge case, exact trigger unconfirmed — first live hit was flirts/generate
  // today), and response.content[0].text.trim() on that shape throws a bare
  // TypeError that every caller's try/catch still caught, but with zero
  // diagnostic info and — worse — skipping logCall entirely, since this line
  // was never inside the block that calls it. That's why today's real
  // failure left no trace in nora_calls at all. Now caught here specifically,
  // logged with the actual stop_reason, and rethrown as a real Error so the
  // next occurrence (in this or any other noraCall-based route) is visible
  // instead of silent.
  ...
  if (!block || typeof block.text !== 'string') {
    await logCall({
      ...
      latencyMs: Date.now() - startedAt,
      status: 'error',
      errorType: 'empty_or_malformed_response',
      errorMessage: `stop_reason=${response.stop_reason}, content_blocks=${response.content?.length ?? 'undefined'}, first_block_type=${block?.type ?? 'none'}`,
    })
    throw new Error(`Nora call returned no usable text content (stop_reason: ${response.stop_reason})`)
  }
  ...
  return noraCall({
    ...
    messages: [{
      ...
      content: [
        ...
        { type: 'text', text: 'Describe what is actually visible in this photo in 1-2 factual sentences — the setting, activity, and any notable objects or details. Plain and observational, not poetic. Simple, factual visual details are fine to include if relevant (e.g. what someone is wearing) — but never make evaluative judgments about how someone looks, their body, or their emotional state, and do not attempt to identify who the people are.' },
      ],
    }],
    system: null,
    context: 'signal',
